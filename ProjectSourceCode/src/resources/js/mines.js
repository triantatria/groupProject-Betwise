...
const Mines = (() => {
  ...
  async function handleStartClick() {
    ...
    if (Number.isNaN(requestedMines) || requestedMines < 1 || requestedMines > GRID_SIZE - 1) {
  ...
  async function handleCashoutClick() {
    ...
    const gross = computePayout(false);
    const payout = Math.max(0, gross - currentBet);
  ...
  async function handleFullClear() {
    ...
    const gross = computePayout(true);
    const payout = Math.max(0, gross - currentBet);
    ...
    try {
      ...
      statusEl.textContent =
        ...
        `Multiplier: bet ${currentBet} × ${multiplier.toFixed(2)}^${GRID_SIZE - numMines}`;
